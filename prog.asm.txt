.data
	user_input: .space 1001  #1001 bytes for 1000 characters and 1 byte for newline
	large_number: .asciiz "too large" #Too large user_input 
	invalid_number: .asciiz "NaN"
	
.text
	main:
										# user_input label stores the hexadecimal numbers from the user
			la $a0, user_input
			li $a1, 1001				#OS expects 1001 bytes from the user
			li $v0, 8					#Call code for reading string 
			syscall
			
			la $s0, user_input	# Saving the user_input to $s0
			li $s1, 0		# Saving the starting pointer $s1 
			li $s2, 0		# Saving the ending pointer $s2
	...
	subprogram_2:	
			...
			la $t7, user_input		# Stores the first address of user_input in $t7
		delete_front:
		...
		# Deletes the spaces at the end of the string
			...
			j end_de
		found1:
			addi $t8, $t8, -1			#subtract 1 from $t8
			j delete_end
		end_de:
